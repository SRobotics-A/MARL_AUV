...
class MARLRiverFlyFollowEnvCfg(DirectMARLEnvCfg):
    ...
    success_velocity_tolerance = 2.0                   # 进入 success zone 的严格速度门槛（enter 条件，从1.5放宽至2.0）
    success_distance_xy_hold: float = 7.0              # 进入后维持 timer 的宽松距离门槛（hold 条件，比 enter 多 1m）
    success_velocity_tolerance_hold: float = 3.0       # 进入后维持 timer 的宽松速度门槛（hold 条件，从2.0放宽至3.0）
